...
**Scope:** Make `auto_distribute` respect `session_limit`. Overflow → Reserve. Not addressed: pinned-aware distribution (spec #3), cross-tier drag-drop (spec #4).
...
- No regression in spec #1 tests (pinned-first ordering still works).
